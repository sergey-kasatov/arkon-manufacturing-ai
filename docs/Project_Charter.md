...
**Four modules beyond CMAPSS publish the section 6 event contract, 2026-09-02.** Two of them are Phase 2 entries and two are roadmap items that landed early, and the distinction matters only to the plan: to the platform they are four modules on one contract. Of the four, **three have been pushed through the deployed Steering Cell rather than only written** - Scania, casting and MVTec. **NEU is the exception**: its 360 events exist as a file and none has ever been posted, so it is the one module whose event stream has not been exercised end to end. Only NHTSA text classification, the last Phase 2 entry, has nothing behind it.
...
**What the four modules after CMAPSS cost the platform: one API projection, and then nothing.** The intake workflow has taken events from the three that were posted, Scania, casting and MVTec, and created their incidents with no change at all, because it validates a contract rather than a domain. NEU is not evidence either way here, because its events were never sent. The status API needed a real fix on the second and third, and it was not the cosmetic one it had been recorded as: it projected `evidence.prediction` as `predicted_rul` for every module, so a Scania failure probability of 0.0373 was served as a remaining useful life of 0.0373 cycles and the assistant reported it as one. The projection now returns the evidence object as published, and that fix is why the fifth module cost nothing: an MVTec incident keeps its own `category`, `ceiling` and `threshold_basis` without anyone touching the API. Nothing failed anywhere in the chain at any point. Detail in `n8n/README.md` and `langflow/README.md`.
