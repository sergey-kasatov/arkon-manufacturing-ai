...
**Built 2026-09-07, on the target this section named.** The queryable store is three n8n data tables kept level with the two logs by one sub-workflow, `Arkon Store Sync v1` (`n8n/store_sync_v1.json`): one row per incident carrying the flat projection and the folded lifecycle, one row per transition, and one summary row with the store-wide numbers and the two line counters the next sync resumes from. The record of truth did not move: the two JSONL logs stay append-only and both write paths still write only them; what each now does in addition is start the sync, without waiting for it, right after its append. The sync folds every incident with the same `lifecycle.py` the transition endpoint enforces, recomputes the summary from that full fold, writes only the rows the last sync had not seen, and appends one line to `/data/arkon/store_sync.jsonl`; a rebuild wipes the tables and is otherwise the same run, so the projection is derivable from the logs by one call, which is what makes it a projection rather than a second record. The status API answers from the rows on the contract of 2026-08-30 unchanged (`n8n/README.md`, "Incident store"): it reads the summary row, the `new` rows for the live overdue count and the rows its one most selective condition names, so a query that used to parse both logs whole now reads what it asked for, measured on the day at 0.16 to 0.19 s against 0.31 to 0.42 s before at a store of 276 incidents and 713 transitions, and the difference grows with the store. Two boundaries are stated rather than hidden. The sync still reads both logs whole, because n8n's file node has no offset read; the cost moved from every read to every write and is bounded by the fold, which took 7 ms on that store, so the next step, if a plant ever needs it, is a sync that reads a tail. And the first sentence of this section, "one SQLite database written by the n8n workflow", is now true in the letter and false in the spirit: the tables live in n8n's SQLite, but the workflow writes the logs and the tables are their projection, and that is the better design, for the reasons section 7.2 gives.
...
**Fixed 2026-09-06, in the shape this section predicted.** The intake outcome is itself an event, and the Steering Cell now writes every one of the three, with its reason, to one intake log, `/data/arkon/intake_outcomes.jsonl`: one line per event carrying the outcome (`recorded`, `rejected`, `duplicate_suppressed`), the reason, the event id, the record id and priority that make the dedup key, the module and domain, the incident id when one was created, whether the alert branch ran and, when it did, Telegram's own message id for the card. The line is written after the caller was answered, so the log can never delay or fail an intake, and the change to the Phase 1 workflow is additive: one node that runs on every branch and two that append its line (`n8n/README.md`, "Intake outcomes"). What the three consequences above become: suppression is countable from the log; a validation regression is a run of `rejected` lines rather than a quiet store; and the assistant still cannot see any of it, because nothing reads the log back yet, which is the next gap and is named here rather than discovered later. Until 2026-09-06 this paragraph said the fix was deliberately not done because Phase 1 was closed; what made it safe to reopen the intake workflow without rewinding its counter is the deployment recipe in `n8n/build/deploy_steering_cell.py`.
