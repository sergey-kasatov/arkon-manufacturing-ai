...
the response-time KPIs computed from timestamps. Written down as well as enforced,
since 2026-09-06: every intake outcome, so that a rejection or a suppression leaves
a line and a validation regression no longer reads as a quiet plant (charter 7.6).
Enforced on the demo engine's side: an alert budget, because every card is real.
